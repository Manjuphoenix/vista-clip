...
class HybridAdaptiveNormalization(nn.Module):
    def __init__(self, num_channels):
        super().__init__()
        # FOr STORING THE NP WEIGHTS
        self.alpha1 = nn.Parameter(torch.ones(4, num_channels, 1, 1), requires_grad=True).float()
        self.beta1 = nn.Parameter(torch.ones(4, num_channels, 1, 1), requires_grad=True).float()
    ...
    def initialize_parameters(self, feat):
        ...
        # ones_mat = torch.ones(1, feat.size(1), 1, 1, device=feat.device)  # Explicitly

        # Create new parameters with proper size
        # WITHOUT Storing NP weights

        # with torch.no_grad():
        #     self.alpha1 = nn.Parameter(torch.normal(ones_mat, 0.75 * ones_mat))
        #     self.beta1 = nn.Parameter(torch.normal(ones_mat, 0.75 * ones_mat))
        ...
        # with torch.no_grad():
        #     self.alpha1.copy_(torch.normal(ones_mat, 0.75 * ones_mat))
        #     self.beta1.copy_(torch.normal(ones_mat, 0.75 * ones_mat))

        print("---------------", self.alpha1.shape, self.beta1.shape, feat_mean.shape, "************************")
    ...
    def forward(self, feat):
        ...
        if self.training:
            output = self.alpha1 * (feat - feat_mean) + self.beta1 * feat_mean
        else:
            output = self.alpha1 * (feat - feat_mean) + self.beta1 * feat_mean
...
#         super().__init__()
#         # FOr STORING THE NP WEIGHTS
#         self.alpha1 = nn.Parameter(torch.ones(1, num_channels, 1, 1), requires_grad=True).float()
#         self.beta1 = nn.Parameter(torch.ones(1, num_channels, 1, 1), requires_grad=True).float()
#         self.initialized = False  # Flag to track initialization
...
class ResNet(Backbone):
    ...
    def __init__(self, stem, stages, num_classes=None, out_features=None, freeze_at=0):
        ...
        # self.han1 = HybridAdaptiveNormalization(stages[0][-1].out_channels)  # After stem
        # self.han2 = HybridAdaptiveNormalization(stages[1][-1].out_channels)  # After res2

        self.patchprompter = FixedPatchPrompter()
