...
#         # self.isize = self.image_size
#         self.psize = 30
        
#         self.isize1 = 640
#         self.isize = [(640, 640), (640, 640), (640, 640), (640, 640)]


#         ############## CORNER PROMPTING ########################
#         # self.patch_tl1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Top-Left
#         # self.patch_tr1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Top-Right
#         # self.patch_bl1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Bottom-Left
#         # self.patch_br1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Bottom-Right
        
        
#         # self.patch_tl = [nn.Parameter(torch.randn([1, 3, self.psize, self.psize])), # Top-Left,
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize]))]
        
#         # self.patch_tr = [nn.Parameter(torch.randn([1, 3, self.psize, self.psize])), # Top-Left,
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize]))]
        
#         # self.patch_bl = [nn.Parameter(torch.randn([1, 3, self.psize, self.psize])), # Top-Left,
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize]))]
        
#         # self.patch_br = [nn.Parameter(torch.randn([1, 3, self.psize, self.psize])), # Top-Left,
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#         #                  nn.Parameter(torch.randn([1, 3, self.psize, self.psize]))]
...
#         ##################### CENTER PROMPTING #####################
#         self.patch = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # center
        
        
#         self.patch_prompt = [nn.Parameter(torch.randn([1, 3, self.psize, self.psize])), # center,
#                          nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#                          nn.Parameter(torch.randn([1, 3, self.psize, self.psize])),
#                          nn.Parameter(torch.randn([1, 3, self.psize, self.psize]))]
        ...
        ############# PAD PROMPTING AROUND THE IMAGE #################
        # self.patch_tl1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Top-Left
        # self.patch_tr1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Top-Right
        # self.patch_bl1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Bottom-Left
        # self.patch_br1 = nn.Parameter(torch.randn([1, 3, self.psize, self.psize])) # Bottom-Right
        ...
        # self.patch_tl = [nn.Parameter(torch.randn([1, 3, self.psize, 640])), # Top-Left,
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640])),
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640])),
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640]))]
        
        # self.patch_tr = [nn.Parameter(torch.randn([1, 3, 640, self.psize])), # Top-Left,
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize])),
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize])),
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize]))]
        
        # self.patch_bl = [nn.Parameter(torch.randn([1, 3, 640, self.psize])), # Top-Left,
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize])),
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize])),
        #                  nn.Parameter(torch.randn([1, 3, 640, self.psize]))]
        
        # self.patch_br = [nn.Parameter(torch.randn([1, 3, self.psize, 640])), # Top-Left,
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640])),
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640])),
        #                  nn.Parameter(torch.randn([1, 3, self.psize, 640]))]
    ...
    #     prompt = [torch.zeros([1, 3, 640, 640]).cuda(),
    #               torch.zeros([1, 3, 640, 640]).cuda(),
    #               torch.zeros([1, 3, 640, 640]).cuda(),
    #               torch.zeros([1, 3, 640, 640]).cuda()]
        
    #     for i in range(3):
    #         # self.image_size[i] = img_size[i]
            
    #         self.isize[i] = image_size[i]
    #         # prompt[i] = torch.zeros([1, 3, 640, 640]).cuda()
    #         if image_size[i][0] > 640 and image_size[i][1] > 640:
    #             continue
            
    #         elif image_size[i][0] <= 640 and image_size[i][1] <= 640:
    #             # print("---------------", prompt[i][:, :, :self.psize, :self.psize].shape, self.patch_tl[i].shape, "**********")
    #             # print("---------------", self.psize, "----width----", self.isize[i][1], "----height-----", self.isize[i][0], "**********")
    #             prompt[i][:, :, :self.psize, :self.psize] = self.patch_tl[i]      # Top-left corner
    #             # prompt[i][:, :,   :self.psize,   640-(self.isize[i][1]-30):640-self.isize[i][1]] = self.patch_tr[i] # Top-right corner
    #             prompt[i][:, :,   :self.psize,  self.isize[i][1]-30:self.isize[i][1]] = self.patch_tr[i] # Top-right corner
    #             # prompt[i][:, :, :self.psize, self.isize[i][1] - self.psize:] = self.patch_tr[i] # Top-right corner
    #             prompt[i][:, :, self.isize[i][0]-30:self.isize[i][0], :self.psize] = self.patch_bl[i] # Bottom-left corner
    #             prompt[i][:, :, self.isize[i][0]-30:self.isize[i][0], self.isize[i][1]-30:self.isize[i][1]] = self.patch_br[i] # Bottom-right corner
                    
    #             x[i] = x[i] + prompt[i]
                
    #         elif image_size[i][0] > 640:
    #         # elif self.image_size[i][0] > 640 or image_size[i][1] > 640:
    #             prompt1 = torch.zeros([1, 3, self.isize1, self.isize1]).cuda()
    #             prompt1[:, :, :self.psize, :self.psize] = self.patch_tl1      # Top-left corner
    #             prompt1[:, :, :self.psize, self.isize[i][1]-30:self.isize[i][1]] = self.patch_tr1 # Top-right corner
    #             prompt1[:, :, 640-30:640, :self.psize] = self.patch_bl1 # Bottom-left corner
    #             prompt1[:, :, 640-30:640, self.isize[i][1]-30:self.isize[i][1]] = self.patch_br1 # Bottom-right corner
                
    #             x[i] = x[i] + prompt1
                
                
    #         elif image_size[i][1] > 640:
    #         # elif self.image_size[i][0] > 640 or image_size[i][1] > 640:
    #             prompt2 = torch.zeros([1, 3, self.isize1, self.isize1]).cuda()
    #             prompt2[:, :, :self.psize, :self.psize] = self.patch_tl1      # Top-left corner
    #             prompt2[:, :, :self.psize, 640-30:640] = self.patch_tr1 # Top-right corner
    #             prompt2[:, :, self.isize[i][0]-30:self.isize[i][0], :self.psize] = self.patch_bl1 # Bottom-left corner
    #             prompt2[:, :, self.isize[i][0]-30:self.isize[i][0], 640-30:640] = self.patch_br1 # Bottom-right corner
    ...
    ######################## AROUND THE IMAGE PAD PROMPTING .....#########################
    # def forward(self, x, image_size):
    ...
    #             # print("---------------", prompt[i][:, :, :self.psize, :self.psize].shape, self.patch_tl[i].shape, "**********")
    #             # print("---------------", self.psize, "----width----", self.isize[i][1], "----height-----", self.isize[i][0], "**********")
    #             prompt[i][:, :, :self.psize, :640] = self.patch_tl[i]      # Top-left corner
    #             # prompt[i][:, :,   :self.psize,   640-(self.isize[i][1]-30):640-self.isize[i][1]] = self.patch_tr[i] # Top-right corner
    #             prompt[i][:, :,   :640,  self.isize[i][1]-30:self.isize[i][1]] = self.patch_tr[i] # Top-right corner
    #             # prompt[i][:, :, :self.psize, self.isize[i][1] - self.psize:] = self.patch_tr[i] # Top-right corner
    #             prompt[i][:, :, :640, :self.psize] = self.patch_bl[i] # Bottom-left corner
    #             prompt[i][:, :, self.isize[i][0]-30:self.isize[i][0], :640] = self.patch_br[i] # Bottom-right corner
    ...
    #         else:
    #             ch = int(image_size[i][0]/2)
    #             cw = int(image_size[i][1]/2)
    #             # print("------------", ch, cw, "****************")
    #             # print(OJIJHOI)

                
    #             prompt[i][:, :, ch-15:ch+15, cw-15:cw+15] = self.patch_prompt[i]      # Top-left corner
    #             x[i] = x[i] + prompt[i]
...
class ResNet(Backbone):
    ...
    def forward(self, x):
            ...
            # cv2.imwrite("center_patching_verify.png", x[0].permute(1,2,0).detach().cpu().numpy()*255)
