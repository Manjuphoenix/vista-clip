...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs, batched_prompts=None):
    # def forward(self, batched_inputs):
        ...
        # print("Model old: ", self.model_old)    # False initially
        # print("training: ", self.training)      # True while training of course...
        # print("per pixel: ", self.per_pixel)    # False initially
        # print("forward inference: ", self.forward_inference)  # The entire model pipeline...
        # print("Criterion: ", self.criterion)    # None initially
        ...
        "desk",
        "rock or stone",
        "wardrobe or closet or press",
        "lamp",
        "tub",
        "rail",
        "cushion",
        "base or pedestal or stand",
        "box",
        "column or pillar",
        "signboard or sign",
        "chest of drawers or chest or bureau or dresser",
        "counter",
        "sand",
        "sink",
        "skyscraper",
        ...
        "grandstand or covered stand",
        "path",
        "stairs",
        "runway",
        "case or display case or showcase or vitrine",
        "pool table or billiard table or snooker table",
        "pillow",
        "screen door or screen",
        "stairway or staircase",
        "river",
        "bridge or span",
        "bookcase",
        "blind or screen",
        "coffee table",
        "toilet or can or commode or crapper or pot or potty or stool or throne",
        "flower",
        "book",
        "hill",
        "bench",
        ...
        "bar",
        "arcade machine",
        "hovel or hut or hutch or shack or shanty",
        "bus",
        "towel",
        "light",
        "truck",
        "tower",
        "chandelier",
        "awning or sunshade or sunblind",
        "street lamp",
        "booth",
        "tv",
        "plane",
        ...
        "clothes",
        "pole",
        "land or ground or soil",
        "bannister or banister or balustrade or balusters or handrail",
        "escalator or moving staircase or moving stairway",
        "ottoman or pouf or pouffe or puff or hassock",
        "bottle",
        "buffet or counter or sideboard",
        ...
        "canopy",
        "washer or automatic washer or washing machine",
        "plaything or toy",
        "pool",
        "stool",
        "barrel or cask",
        "basket or handbasket",
        "falls",
        "tent",
        "bag",
        "minibike or motorbike",
        "cradle",
        "oven",
        "ball",
        "food or solid food",
        ...
        # template = "This photo contains {}"
        
        
        segs = [x["segments_info"] for x in batched_inputs]
        ...
        for j in range(len(segs)):
            classes = []
            for i in segs[j]:
                # TO check if the class already occured in the image
                if cls_list[i["category_id"]] in classes:
                    continue
                else:
            # This will give the text embedding of shape 1, 256...
            #         classes.append(cls_list[i["category_id"]])
            # classes_batch = classes_batch + classes
            
                    
            # This will output the batch of all classes text embeddings..
                    classes.append(cls_list[i["category_id"]])
                    # print("***********", classes, "__________")
            classes_batch.append(classes)
            
        # print("***********", classes_batch, "__________")
        # print(OHOIH)
            # print("-------------", i, "**************")
            # print("---------------", cls_list[i], "************")
        # print("---------------", classes, "************")  # Contains only the class names of objects present in the given image
        
        tmp_feat = torch.ones(len(classes), 256)
        
        idx = []
        
        for k in classes:
            idx.append(cls_list.index(k))
        
        # print("**********", idx, classes[0], "_-------------_")
        # print("-------------", tmp_feat.shape, "**********")
        # print(JPOIIOJ)
        
        # print(classes)
        
        # print("(_________________-------------)", batched_inputs, "_____________")
        # print("(_________________-------------)", batched_inputs[0]["image"].shape, "_____________")
        # print("(_________________-------------)", batched_inputs[0]["image"].permute(1,2,0).cpu().numpy().shape, "_____________")
        
        # import cv2
        # cv2.imwrite("/home/wirin/manjunath/EClipse_backup/verify.png", batched_inputs[0]["image"].permute(1,2,0).cpu().numpy())
        # print("---------------", batched_inputs, "_-------------_")
        
        # print(HOHI)
        
        if self.training:

            features, txt_features = self.backbone(images.tensor, classes_batch)     # FOR CLIP VLM
            # features, txt_features = self.backbone(images.tensor, batched_prompts)     # FOR CLIP VLM
            # txt_features = txt_features.unsqueeze(-1).unsqueeze(-2)
            # features["res4"] = features["res4"] + txt_features
            # print("-------------------", features["res4"].shape, txt_features.shape, "-------------------")
            # features = self.backbone(images.tensor)     # FOR Normal ECLIPSE
        
        else:
            features, txt_features = self.backbone(images.tensor, cls_list)     # FOR Normal ECLIPSE
            # txt_features = txt_features.unsqueeze(-1).unsqueeze(-2)
        
        # txt_features = None
        # print("--------------txt feat-------------", txt_features.shape)
        # print(HOY)
        outputs = self.sem_seg_head(features, txt_features, classes_batch)
        del classes
        del classes_batch
        # print("----------------", outputs, "-------------------")
        # print(IHUOHOI)
    ...
    def freeze_for_prompt_tuning(self, ):
        ...
        if not self.model_old:
            ...
            if self.backbone_freeze:
                # for k, p in self.backbone.visual.patchprompter.patch():
                #     p.requires_grad = True
                ...
                # self.backbone.visual.han2.alpha1.requires_grad = True
                # self.backbone.visual.han2.beta1.requires_grad = True
            #####################################################################################
            ...
            ############## For learnable weighted sum ##################
            # self.sem_seg_head.predictor.alpha.requires_grad = True
            ############################################################
